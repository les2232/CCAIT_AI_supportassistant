...
def llm_model_name():
    return os.environ.get("IT_SUPPORT_LLM_MODEL", DEFAULT_LLM_MODEL).strip() or DEFAULT_LLM_MODEL
...
def polish_answer(question, section_heading, article_id, answer_text):
    """
    Rewrite a retrieved answer into clearer student-facing language using only the supplied source text.
    ...
    instructions = (
        ...
        "Do not use general knowledge. Do not invent URLs, policies, phone numbers, steps, account rules, "
        "or procedures not present in the source text. Keep the answer concise, clear, calm, and student-friendly. "
        "Do not mention that you are rewriting or summarizing. If the source text does not contain enough detail, "
        "say that the student should contact CCA IT Support."
    )
    source_payload = (
        f"Question:\n{question.strip()}\n\n"
        f"Article ID:\n{(article_id or '').strip()}\n\n"
